...
~~~
Мы можем также перенести все тесты по проверке Ip  в отдельный тест.
```php
...
```
~~~
Сохранение в Git:  Тест для объекта-значения
~~~
У нас есть одна проблема, что кто-то может отнаследоваться от класса Ip и переопределить его конструктор.
Чтобы такого не было, то мы можем обозначить этот класс как финальный:
```php
final class Ip {...}
```
Но мы не будем финализировать класс, а договоримся с разработчиком, что менять его нельзя.
~~~
Сохранение в Git:  Защита через запрет наследования
~~~
После выноса тестов по Ip, мы можем изменить тест класса Locator.
```php
class LocatorTest extends TestCase
{
    public function testSuccess(): void
...
```
Сейчас тесты реально ходят в Интернет, в следующем эпизоде мы рассмотрим способ этого избежать.
~~~
Сохранение в Git:  Разделение тестов
~~~
